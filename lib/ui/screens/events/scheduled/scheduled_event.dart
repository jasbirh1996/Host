...
class ScheduledEvent extends StatefulWidget {
  ...
  @override
  State<ScheduledEvent> createState() => _OnGoingEventState();
}

class _OnGoingEventState extends State<ScheduledEvent> {
  var controller = Get.put(EventController());
  @override
  void initState() {
    controller.getScheduledData(
      callback: (){
        controller.dropdownList.clear();
        controller.dropdownList = ['Select an event'];
        setState(() {
          controller.dropdownList.addAll(controller.scheduleList.map((e) => e.eventName).toList());
      ...
      body: Container(
        child: Column(
          children: [
            ...
            Expanded(
              child: ListView.builder(itemCount: controller.scheduleList.length,itemBuilder: (BuildContext context, int index)
              {
                var data = controller.scheduleList[index];
                return Column(
                  children: [
                    Container(
                        ...
                        children: [
                          Expanded(child: Container(
                              ...
                              child: AppComponents.getTransparentButton("View Table", () {
                                Fluttertoast.showToast(msg: "Ceremony not yet started");
