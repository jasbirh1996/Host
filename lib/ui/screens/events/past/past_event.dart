...
class PastEvent extends StatefulWidget {
  ...
  @override
  State<PastEvent> createState() => _OnGoingEventState();
}

class _OnGoingEventState extends State<PastEvent> {
  var controller = Get.put(EventController());
  @override
  void initState() {
    controller.dropdownList.clear();
    controller.dropdownList = ['Select an event'];
    controller.getPastData(
        callback: (){

          setState(() {
            controller.dropdownList.addAll(controller.pastList.map((e) => e.eventName).toList());
      ...
      body: Container(
        child: Column(
          children: [
            ...
            Expanded(
              child: ListView.builder(itemCount: controller.pastList.length,itemBuilder: (BuildContext context, int index)
              {
                var data = controller.pastList[index];
                return Column(
                  children: [
                    Container(
                        ...
                        children: [
                          ...
                          Container(
                              ...
                              child: AppComponents.getTransparentButton("View Table", () {
                                controller.getCeremonyDetail(data.id,callback: (){
                                  if(controller.ceremonyDetail!.tabledata.isNotEmpty){
                                    Get.to(TableView(eventName: "",ceremonyId: data.id,));

                                  }else{
                                    Fluttertoast.showToast(msg: "No table data available");
                          ...
                          Container(
                              ...
                              child: AppComponents.createButton("View Menu", () {
                                ...
                                controller.getMenu(data.id,callback: (menu){
                                  if(menu.isNotEmpty){
                                    Get.to(MenuView(ceremonyId: data.id,));
                                  }else{
                                    Fluttertoast.showToast(msg: "No menu available");
                                  }
                                });
            ...
            controller.pastList.isNotEmpty?
            Container(
                margin: EdgeInsets.only(left: 20,right: 20),
                height: 48,
                child: AppComponents.createButton("Rate Service", () {
            ...
            Container(
              margin: EdgeInsets.only(left: 20,right: 20),
                height: 48,
                child: AppComponents.getTransparentButton("Download Event Log", () {
                  controller.downloadEventLog();

                },defaultfont: AppFonts.openSansSemiBold)):Container(),
